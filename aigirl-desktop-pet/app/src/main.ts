...
async function triggerInteraction(id: string): Promise<InteractionResult> {
  ...
  if (interaction.id === 'walk') {
    startWalkAnimation(interaction.durationMs);
  } else {
...
let walkStopTimer: ReturnType<typeof setTimeout> | undefined;

function stopWalkAnimation(): void {
...
function startWalkAnimation(durationMs: number): void {
  stopWalkAnimation();
  let direction = 1;
  sendActivity({ kind: 'walk', stateId: 'walk', durationMs: 700 });
  walkTimer = setInterval(() => {
    if (!petWindow || petWindow.isDestroyed() || dragSession) return;
    const bounds = petWindow.getBounds();
    const workArea = screen.getDisplayMatching(bounds).workArea;
    let x = bounds.x + direction * 42;
    if (x <= workArea.x + 16) { direction = 1; x = workArea.x + 16; }
    if (x + bounds.width >= workArea.x + workArea.width - 16) { direction = -1; x = workArea.x + workArea.width - bounds.width - 16; }
    petWindow.setPosition(Math.round(x), bounds.y, false);
    sendActivity({ kind: 'walk', stateId: direction > 0 ? 'run-right' : 'run-left', durationMs: 600 });
  }, 600);
  walkStopTimer = setTimeout(() => {
    stopWalkAnimation();
    sendActivity({ kind: 'idle', stateId: stateForTrigger('ambient:idle')?.id, durationMs: 900 });
  }, durationMs);
}

// —— 跟随鼠标模式 ——
let followMouseEnabled = false;
let followMouseTimer: ReturnType<typeof setInterval> | undefined;

function setFollowMouse(enabled: boolean): void {
  followMouseEnabled = enabled;
  if (enabled) {
    ...
    followMouseTimer = setInterval(() => {
      ...
      const targetX = cursor.x - Math.round(bounds.width / 2);
      const targetY = Math.round(Math.max(workArea.y, cursor.y - Math.round(bounds.height * 0.85)));
      const dx = targetX - bounds.x;
      const dy = targetY - bounds.y;
      if (Math.abs(dx) < 6 && Math.abs(dy) < 6) {
        if (lastStateId !== undefined && lastStateId !== 'idle') {
          lastStateId = 'idle';
          sendActivity({ kind: 'idle', stateId: stateForTrigger('ambient:idle')?.id, durationMs: 900 });
        }
        return;
      }
      const nextX = Math.round(bounds.x + dx * 0.35);
      const nextY = Math.round(bounds.y + dy * 0.35);
      petWindow.setPosition(nextX, nextY, false);
      const stateId = dx > 0 ? 'run-right' : 'run-left';
      if (lastStateId !== stateId) {
        lastStateId = stateId;
        sendActivity({ kind: 'movement', stateId, durationMs: 400 });
      }
    }, 80);
  } else {
    if (followMouseTimer) clearInterval(followMouseTimer);
    followMouseTimer = undefined;
...
function toggleFollowMouse(): void {
  setFollowMouse(!followMouseEnabled);
}
...
function buildPetMenu(): Electron.MenuItemConstructorOptions[] {
  ...
  items.push({ label: '⏰ 添加提醒', click: showReminderComposer });
  items.push({ label: followMouseEnabled ? '🧲 跟随鼠标：开' : '🧲 跟随鼠标：关', click: toggleFollowMouse });
  items.push({
    label: '📏 调整大小',
    submenu: [
      { label: '小', type: 'radio', checked: settings.petScale === 0.65, click: () => void saveSettings({ ...settings, petScale: 0.65 }) },
      { label: '默认', type: 'radio', checked: settings.petScale === 0.8, click: () => void saveSettings({ ...settings, petScale: 0.8 }) },
      { label: '大', type: 'radio', checked: settings.petScale === 1, click: () => void saveSettings({ ...settings, petScale: 1 }) },
      { label: '超大', type: 'radio', checked: settings.petScale === 1.2, click: () => void saveSettings({ ...settings, petScale: 1.2 }) },
    ],
  });
  items.push({ label: settings.alwaysOnTop ? '📌 置顶：开' : '📌 置顶：关', click: () => void saveSettings({ ...settings, alwaysOnTop: !settings.alwaysOnTop }) });
  items.push({ type: 'separator' });
  items.push({ label: '🙈 隐藏桌宠', click: () => petWindow?.hide() });
  items.push({ label: '🚪 退出程序', click: () => { isQuitting = true; app.quit(); } });
  return items;
}
...
function registerIpc(): void {
  ...
  ipcMain.handle('settings:update', async (event, patch: unknown) => {
    assertSender(event, ['dashboard', 'pet']);
